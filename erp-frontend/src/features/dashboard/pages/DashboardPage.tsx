...
export default function DashboardPage() {
  ...
  const dashboardQuery = useQuery({
    ...
    retry: 1,
    staleTime: 60_000,          // serve from cache for 60s on revisit — instant load
    gcTime: 5 * 60_000,         // keep cache warm for 5 minutes after unmount
    refetchOnWindowFocus: false, // avoid spurious re-fetches on alt-tab
  });

  const summary = dashboardQuery.data;
  const lowStockItems = summary?.inventory.lowStockItems ?? [];
  const ordersByStatus = summary?.orders.byStatus ?? [];
  const totalOrdersByStatus = ordersByStatus.reduce((total, s) => total + s.value, 0);
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 gap-3 xs:grid-cols-2 md:grid-cols-3 xl:grid-cols-6">
        <KPICard title="Total Products"      value={summary?.counts.products ?? 0}                        icon={Package}       description="catalog" />
        <KPICard title="Available Inventory" value={(summary?.inventory.availableQuantity ?? 0).toLocaleString()} icon={Boxes}        description="sellable" />
        ...
        <KPICard title="Active Orders"       value={summary?.orders.activeCount ?? 0}                     icon={ShoppingCart}  description="open flow" />
        <KPICard title="Draft Purchases"     value={draftPurchaseCount}                                    icon={Truck}         variant={draftPurchaseCount > 0 ? 'warning' : 'default'} description="awaiting action" />
        <KPICard title="Customers"           value={summary?.counts.customers ?? 0}                       icon={Users}         description="accounts" />
        <KPICard title="Suppliers"           value={summary?.counts.suppliers ?? 0}                       icon={Factory}       description="vendors" />
        <KPICard title="Warehouses"          value={summary?.counts.warehouses ?? 0}                      icon={Warehouse}     description="locations" />
      </div>
          ...
          <div className="space-y-2">
            {(summary?.orders.recent ?? []).map((order) => (
              <Link
                key={order.id}
                to={`/orders/${order.id}`}
                className="flex flex-col gap-2 rounded-lg p-3 transition-colors hover:bg-muted/30 xs:flex-row xs:items-center xs:justify-between"
              >
      ...
      <div className="erp-card p-5">
        ...
        <div className="space-y-2">
          {lowStockItems.slice(0, 5).map((item) => (
            <div
              key={item.id}
              className="flex flex-col gap-2 rounded-lg border border-warning/20 bg-warning/10 p-3 xs:flex-row xs:items-center xs:justify-between"
            >
              <div className="min-w-0">
